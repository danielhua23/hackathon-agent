...
@triton.autotune(configs=[triton.Config({'BLOCK_SIZE': 64}, num_stages=1, num_warps=2), triton.Config({'BLOCK_SIZE': 128}, num_stages=1, num_warps=4), triton.Config({'BLOCK_SIZE': 256}, num_stages=1, num_warps=8), triton.Config({'BLOCK_SIZE': 512}, num_stages=1, num_warps=8)], key=[])
@triton.jit
def kernel_function(x_ptr, output_ptr, n_elements, BLOCK_SIZE: tl.constexpr):
    ...
    x_sin = tl.sin(x)
    tl.store(output_ptr + offsets, x_sin.to(output_ptr.dtype.element_ty), mask=mask)
