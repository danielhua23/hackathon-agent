...
def _l2_norm_fwd_1pass_kernel(X, Y, stride_x_row, N, eps, BLOCK_N: tl.constexpr):
    ...
    mask = cols < N
    offset = row * stride_x_row + cols
    x = tl.load(X + offset, mask=mask, other=0.0).to(tl.float32)
    var = tl.sum(x * x, axis=0)
    rstd = tl.rsqrt(var + eps)
    y = x * rstd
    tl.store(Y + offset, y.to(Y.dtype.element_ty), mask=mask)

def _l2_norm_fwd(x: torch.Tensor, eps: float=1e-05) -> torch.Tensor:
    x_shape_og = x.shape
    x = x.view(-1, x_shape_og[-1]).contiguous()
    y = torch.empty_like(x)
    M, N = x.shape
    max_elem_size = 65536 // x.element_size()
    BLOCK_N = min(max_elem_size, triton.next_power_of_2(N))
    if N > BLOCK_N:
        raise RuntimeError('Feature dim >= 64KB or too large for 1-pass kernel')
    _l2_norm_fwd_1pass_kernel[M,](x, y, x.stride(0), N, eps, BLOCK_N=BLOCK_N, num_warps=4)
