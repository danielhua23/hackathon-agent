...
def _l2_norm_fwd(x: torch.Tensor, eps: float=1e-05) -> torch.Tensor:
    ...
    if BLOCK_N * element_size > max_block_bytes:
        BLOCK_N = max_block_bytes // element_size
    if N > BLOCK_N:
        raise ValueError(f'Feature dimension {N} exceeds max block size {BLOCK_N} after memory limit')
    _l2_norm_fwd_1pass_kernel[M,](x, y, x.stride(0), N, eps, BLOCK_N=BLOCK_N)
    return y.view(origin_shape)
