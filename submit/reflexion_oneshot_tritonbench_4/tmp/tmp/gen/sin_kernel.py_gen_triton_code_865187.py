...
@triton.autotune(configs=[triton.Config({'BLOCK_SIZE': 512}, num_warps=2, num_stages=1), triton.Config({'BLOCK_SIZE': 512}, num_warps=4, num_stages=1), triton.Config({'BLOCK_SIZE': 1024}, num_warps=4, num_stages=1), triton.Config({'BLOCK_SIZE': 1024}, num_warps=8, num_stages=1)], key=['n_elements'])
@triton.jit
...
def call_kernel(x: torch.Tensor):
    assert x.is_contiguous() and x.dtype in (torch.float16, torch.float32)
    output = torch.empty_like(x)
    n_elements = x.numel()
    grid = lambda meta: (triton.cdiv(n_elements, meta['BLOCK_SIZE']),)
    kernel_function[grid](x, output, n_elements)
    return output
