...
def _l2_norm_fwd_1pass_kernel(X, Y, stride_x_row, N, eps, BLOCK_N: tl.constexpr):
    ...
    for off in range(0, N, BLOCK_N):
        mask = off + cols < N
        x = tl.load(X + off + cols, mask=mask, other=0.0)
        y = x * rstd
        tl.store(Y + off + cols, y, mask=mask)

def _l2_norm_fwd(x: torch.Tensor, eps: float=1e-12):
    x_shape_og = x.shape
    x = x.reshape(-1, x.shape[-1]).contiguous()
    y = torch.empty_like(x)
    M, N = x.shape
    element_size = x.element_size()
    max_block_n = 65536 // element_size
    BLOCK_N = min(max_block_n, triton.next_power_of_2(N))
    if N > max_block_n:
        raise RuntimeError('This L2 norm only supports feature dim < 64KB.')
    grid = (M,)
