...
def _l2_norm_bwd_kernel(X, DY, DX, stride_x_row, N, eps, BLOCK_N: tl.constexpr):
    ...
    x_square = tl.where(mask, x * x, 0.0)
    var = tl.sum(x_square, axis=0)
    rstd = 1.0 / tl.sqrt(var + eps)
    dy_x = tl.where(mask, dy * x, 0.0)
    sum_dy_x = tl.sum(dy_x, axis=0)
    term = 1.0 / (var + eps) * rstd * sum_dy_x
    dx = dy * rstd - term * x
...
def _l2_norm_bwd(x: torch.Tensor, dy: torch.Tensor, eps: float=1e-05):
    ...
    dy = dy.reshape(x.shape[0], dy.shape[-1])
    if x.stride(-1) != 1:
        x = x.contiguous()
    if dy.stride(-1) != 1:
        dy = dy.contiguous()
